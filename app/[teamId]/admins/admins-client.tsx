...
export function AdminsClient({ teamId, currentUserId, currentRole, initialMembers }: AdminsClientProps) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-100 to-slate-200">
      <div className="mx-auto max-w-6xl p-4 md:p-6">
        <h1 className="mb-6 text-xl font-bold text-slate-800">管理者一覧</h1>
        ...
        {isInviting && (
          <div className="mb-6 rounded-xl bg-white p-4 shadow-md">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-bold text-slate-800">管理者を招待</h2>
              <button onClick={() => setIsInviting(false)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600">
                <X className="h-5 w-5" />
              </button>
            </div>
              ...
              {members.map((member) => (
                <div key={member.id} className="flex items-center gap-4 px-4 py-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-500">
                    {member.role === "owner" ? <ShieldCheck className="h-5 w-5" /> : <Shield className="h-5 w-5" />}
                  </div>
