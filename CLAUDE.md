...
- セクション間隔: 設定画面などで `border border-border p-6` の `<section>` を縦に並べるレイアウトでは、各セクションに `mb-6` を付けて次のセクションとの間隔を確保する。既存コードは末尾のセクションだけ `mb-6` を省略していることがあるため、その直後に新しいセクションを追加するときは追加先セクションに `mb-6` があるか必ず確認する（無ければ追加する）。実装後は該当画面のレンダリング結果（HTML/スクリーンショット等）を見て隣接セクションの間隔を目視確認する
